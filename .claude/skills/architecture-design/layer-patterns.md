# 수직 4대 레이어 격리 모범 및 안티패턴 코드 표준

이 문서는 [`./SKILL.md`](./SKILL.md)가 정의하는 아키텍처 거버넌스를 준수하는 백엔드 애플리케이션의 수직 데이터 흐름 레이어 격리 구현 표준과 안티패턴을 제시한다.
...
RAG 시스템 개발 시, Repository 계층이 벡터 DB의 검색 결과에 시스템 프롬프트 조각(Context String)을 감싸 조립해 반환하는 행위는 프롬프트 엔지니어링이 인프라 데이터 계층에 강결합되게 하므로 금지한다.
```python
