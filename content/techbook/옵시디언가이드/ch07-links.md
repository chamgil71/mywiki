...
② "링크 연습" 노트에서 [[나의 첫 번째 노트#오늘 배운 것]] 입력
...
### 3.4 별칭(Alias) `[[노트명|표시텍스트]]`

링크의 표시 텍스트를 노트 이름과 다르게 지정합니다.

```markdown
<!-- 파일명은 길지만 본문에서는 짧게 표시 -->
[5장](/techbook/옵시디언가이드/ch05-markdown-basic)
[[나의 첫 번째 노트|첫 노트]]

<!-- 문장 흐름에 자연스럽게 삽입 -->
이 내용은 [[제텔카스텐 원리|제텔카스텐]]에서 비롯됩니다.
자세한 설명은 [이 챕터](/techbook/옵시디언가이드/ch07-links)를 참고하세요.
```
...
---
```
YAML aliases에 등록된 이름으로도 `[[링크챕터]]` 처럼 이 노트를 참조할 수 있습니다.
...
**백링크의 두 가지 섹션:**
- **링크된 멘션(Linked Mentions):** `[[직접 링크]]` 로 연결된 노트
...
## 배경
이 프로젝트는 [[고객 요구사항 정리]]에서 출발했습니다.
기술 스택은 [[기술 검토 메모#최종 결정]]을 참고하세요.

## 관련 노트
- [[경쟁사 분석 2024]]
- [[예산 계획서^승인된-예산]] ← 특정 단락 직접 참조
```
...
### Q2. 노트 이름을 바꿨더니 링크가 `[[원래 이름]]` 으로 유지됩니다
...
- [ ] 기본 내부 링크 `[[노트명]]` 을 만들고 이동해봤다
- [ ] 앵커 링크 `[[노트명#제목]]` 을 사용해봤다
- [ ] 별칭 `[[노트명|표시텍스트]]` 를 사용했다
