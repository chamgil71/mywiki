...
**내부 링크**는 `[노트 이름](/노트 이름)` 형식으로 노트끼리 연결하는 핵심 기능입니다.  
**그래프 뷰**는 연결된 노트들의 관계망을 시각적으로 보여줍니다.

### 파일 저장 방식 비유

> 옵시디언의 보관소는 **물리적인 서랍장**과 같습니다. 서랍장(보관소) 안에 종이(`.md` 파일)를 넣는데, 각 종이에는 다른 종이를 참조하는 메모(`[링크](/링크)`)가 붙어 있습니다. 어떤 회사가 서랍장 제조를 중단해도 종이는 사라지지 않습니다.
