# LLM 파이프라인 개발 표준 및 실물 구현 가이드 (llm_pipeline_development.md)

이 문서는 AI-PMO 개발 거버넌스(`04_development.md`)를 준수하는 LLM 연동 파이프라인(Pydantic, Prompt Engine, Input/Output Guards)의 모범 코드 구현 가이드이다.
...
LLM의 비결정적 출력이 정형화된 JSON 형식을 준수하도록 보장하며, 주민등록번호, 휴대전화, 이메일 등의 기밀 데이터(PII)를 전송 전에 필터링 마스킹한다.
...
# 1. 출력 가드용 개인정보(PII) 마스킹 패턴
PII_PATTERNS = {
    "rrn": r"\d{6}-[1-4]\d{6}",             # 주민등록번호
    "phone": r"01[0-9]-\d{3,4}-\d{4}",       # 휴대전화
...
# 2. 출력 JSON 형식 검증 및 500ms 성능 사양 충족을 위한 Fail-Safe Fast Return 파이프라인
def output_guard_pipeline(llm_client, prompt: str, max_retries: int = 1) -> Dict[str, Any]:
    ...
    # API 레이턴시 500ms 사양 충족을 위해 동기 루프 내 재시도는 최대 1회로 통제
    while retry_count <= max_retries:
        try:
            # 타임아웃 3.0초 지정 호출 (LLM의 행 현상 방지)
            raw_response = llm_client.call(prompt, timeout=3.0)
            masked_response = mask_pii_data(raw_response)
            
            # 마크다운 코드 블록 마커 제거 후 파싱
            ...
            if retry_count > max_retries:
                break
            # 오류 메시지를 추가하여 Self-Correction 유도
            prompt += f"\n\n[Warning] JSON 파싱 실패. 철저히 JSON만 출력하십시오: {str(e)}"
            
    # 최종 실패 시 백그라운드 로깅 후 즉각 Fallback 구조체 반환 (성능 락 해제)
