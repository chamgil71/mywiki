...
1.  **템플릿 복사(대형 프로젝트 선택)**: 기획 단계(`docs/spec.md`)가 승인되면, 에이전트는 `agent/templates/docs/architecture.md`를 **`docs/architecture.md`**로 복사한다. (소규모는 `docs/spec.md` 아키텍처 섹션으로 갈음)
2.  **아키텍처 명세서 작성**: 시스템의 레이어 관계와 물리 구조를 상세히 기록한 뒤 `docs/spec.md` 아키텍처 파트에 동기화하고 사용자 승인을 득한다.
